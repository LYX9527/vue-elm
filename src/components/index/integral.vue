<template>
  <div id="cont">
    <div class="nav">
      ...
      <span class="nav_title">我的积分</span>
      <div class="ic"></div>
    </div>
    ...
    <div class="showbalance">
      <div>
        <div>
          <div>当前积分</div>
          <div @click="skip('questiondetail', 3)">
            <icon type="tips" />积分说明
          </div>
        </div>
        <div><span>0</span>分</div>
        <div>
          <van-button type="warning" block>积分兑换商品</van-button>
        </div>
      </div>
    </div>
    <div class="body-title">最近30天积分记录</div>
    <div class="body">
      <div>
        <img
          ...
          alt=""
        />
        <p style="font-size: 1rem">最近30天无积分记录</p>
        <p style="font-size: 0.7rem">快去下单赚取大量积分吧</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...mapMutations(["chNowMsg"]),
    skip(address: string, id: number) {
      this.chNowMsg(id);
